...
- **🔒 开源**: 未提及
- **中文摘要**: 对软体机器人等高维系统的控制，需要既能精确捕捉复杂动力学特性又保持计算可行性的模型。本研究提出一种将基于图神经网络（GNN）的动力学模型与结构利用模型预测控制相结合的框架，实现对高维系统的实时控制。通过将系统表示为具有局部交互作用的图结构，GNN保持了系统的稀疏性，而定制化的凝聚算法则从控制问题中消除状态变量，确保计算效率。该凝聚算法的复杂度随系统节点数量呈线性增长，并利用图形处理器（GPU）并行化实现实时性能。所提方法在仿真和物理软体机器人躯干实验中均得到验证。结果表明，该方法可扩展至1000个节点系统在100赫兹闭环频率下运行，并在硬件上实现亚厘米精度的实时参考轨迹跟踪，性能较基线方法提升63.6%。最后，我们展示了该方法实现有效全身避障的能力。
...
- **🔒 开源**: 未提及
- **中文摘要**: 我们提出闪烁多臂赌博机（FMAB）这一新型MAB框架，其中可用臂集（或行动集）每轮动态变化，且任意时刻的可用集合可能取决于智能体先前选择的臂。我们通过随机图过程对这种受限的演化可用性进行建模，其中臂对应节点，智能体的移动被限制在其局部邻域内。我们在两种随机图模型下分析该问题：独立同分布的埃尔德什-雷尼（ER）过程和边马尔可夫过程。我们提出并分析了一种两阶段算法，该算法采用惰性随机游走进行探索以高效识别最优臂，随后通过导航与锁定阶段进行利用。我们为两种图设置建立了高概率与期望次线性遗憾界。通过为该问题类建立匹配的信息论下界，我们证明算法探索成本接近最优，这揭示了局部移动约束下探索的根本成本。我们通过数值模拟补充理论保证，包括地面机器人侦察灾区的应用场景。
...
- **🔒 开源**: 未提及
- **中文摘要**: 现有基于可验证奖励的强化学习（RLVR）算法（如GRPO）依赖于僵化、均匀且对称的置信域机制，这种机制与大型语言模型（LLMs）复杂的优化动态存在根本性错配。本文指出这些方法存在三个关键挑战：（1）硬截断的二元截断机制导致梯度利用效率低下；（2）均匀比率约束忽视词元分布特性，引发概率质量不敏感问题；（3）正负样本间信用分配模糊度差异导致信号可靠性不对称。为弥合这些差距，我们提出质量自适应软策略优化（MASPO）这一统一框架，旨在协调这三个维度。MASPO集成了可微分软高斯门控机制以最大化梯度效用，采用质量自适应限幅器平衡概率谱上的探索能力，并通过非对称风险控制器使更新幅度与信号置信度对齐。大量实验评估表明，MASPO作为鲁棒的一体化RLVR解决方案，显著超越了现有强基线方法。代码已开源：https://anonymous.4open.science/r/ma1/README.md。
...
- **🔒 开源**: 未提及
- **中文摘要**: 在先前访问过的环境中运行正变得对自主系统日益重要，这直接应用于自动驾驶、测绘以及仓库或家庭机器人技术。这种对同一区域的重复观测给建图与定位——实现任何高级任务的关键环节——带来了重大挑战。本研究提出了一种新颖的多会话框架，该框架基于地图定位构建，不同于常见的贪婪式运行完整SLAM会话并试图在生成的地图间寻找对应关系的做法。我们的方法引入了基于拓扑感知与不确定性评估的决策机制，通过分析位姿图结构来检测低连通性区域，并选择性触发建图与闭环检测模块。生成的位姿图与地图被无缝整合到现有模型中，有效减少累积误差并提升全局一致性。我们在数据集的重叠序列上验证了该方法，并在真实矿井类环境中证明了其有效性。
...
- **🔒 开源**: 未提及
- **中文摘要**: 生成式人工智能正在重塑知识工作，但现有研究主要聚焦软件工程与自然科学领域，针对人文社会科学的方法论探索尚显不足。本研究作为一项"方法论实验"，提出适用于人文社会科学研究的AI智能体协同工作流程（Agentic Workflow）。研究以Anthropic经济指数（AEI）中台湾地区Claude.ai使用数据（2025年11月，N=7,729组对话）为实证载体，验证该方法的可行性。

本研究在两个层面展开：主要层面是方法论框架的设计与验证——构建基于任务模块化、人机分工、可验证性三原则的七阶段模块化工作流，每个阶段明确人类研究者（研究判断与伦理决策）与AI智能体（信息检索与文本生成）的角色分工；次要层面是对AEI台湾数据的实证分析——作为工作流应用于二手数据研究的操作示范，同时展现研究过程与成果质量（参见附录A）。

本研究的贡献在于：为人文社会科学研究者提出可复现的AI协作框架，并通过操作过程的反思性记录，识别出人机协作的三种操作模式——直接执行、迭代优化、人类主导。这种分类揭示了人类在研究问题构建、理论阐释、情境化推理及伦理反思中不可替代的判断作用。研究同时承认了单平台数据、横截面设计及AI可靠性风险等局限性。
...
- **🔓 开源**: CODE
- **中文摘要**: 灾害强度和频次的持续攀升使得应急救援人员的工作愈发艰巨。人工智能与机器人技术有望缓解其操作压力，应对这些挑战。为此，我们提出一套专为应急救援人员设计的基于手势的无人地面车辆控制数据集，通过借鉴现有应急救援手势及战术手语体系，并结合资深救援人员的反馈意见，最终形成包含12种指令的手势系统。数据采集工作共获得3312组RGBD双模态数据，涵盖2个拍摄视角与7种不同距离。据我们所知，这是首个专门针对应急救援人员手势引导无人地面车辆而构建的数据集。我们进一步为这个命名为FR-GESTURE的RGBD数据集制定了评估标准，并开展了基线实验以供后续优化参考。为促进该领域研究发展，我们已公开全部数据：https://doi.org/10.5281/zenodo.18131333。
