...
- **🔒 开源**: 未提及
- **中文摘要**: 对软体机器人等高维系统的控制，需要既能精确捕捉复杂动态特性，又能保持计算可行性的模型。本研究提出一种将基于图神经网络（GNN）的动态模型与结构利用模型预测控制相结合的框架，实现对高维系统的实时控制。通过将系统表示为具有局部交互作用的图结构，GNN保持了模型的稀疏性，同时采用定制的凝聚算法从控制问题中消除状态变量，确保计算效率。该凝聚算法的复杂度随系统节点数量呈线性增长，并利用图形处理器（GPU）并行化实现实时性能。所提方法在仿真和物理软体机器人躯干实验中均得到验证。结果表明，该方法可扩展至1000个节点系统，在100赫兹闭环控制下运行，并在硬件上实现亚厘米精度的实时参考轨迹跟踪，性能较基线方法提升63.6%。最后，我们展示了该方法实现有效全身避障的能力。
...
- **🔒 开源**: 未提及
- **中文摘要**: 现有空中机器人导航系统通常围绕静态与动态障碍物规划路径，但当静态障碍物突然移动时无法及时适应。通过融入环境语义感知能力，可对突然移动障碍物带来的潜在风险进行预估。本文提出RA-Nav——基于语义分割的风险感知导航框架。通过轻量化多尺度语义分割网络实时识别障碍物类别，进一步将障碍物划分为静止型、暂时静止型与动态型三类，针对每类障碍物设计相应风险估计函数实现实时风险预测，并据此构建完整的局部风险地图。基于该地图设计风险感知路径搜索算法，确保规划出兼顾路径效率与安全性的路径，再通过轨迹优化生成安全、平滑且动力学可行的轨迹。对比仿真实验表明，在突发障碍物状态转换场景中，RA-Nav相比基线方法获得更高成功率。通过真实数据仿真进一步验证了其有效性。
...
- **🔒 开源**: 未提及
- **中文摘要**: 在已访问环境中进行操作对自主系统而言正变得日益重要，其在自动驾驶、测绘以及仓储或家庭机器人等领域具有直接应用价值。这种对同一区域的重复观测给建图与定位——实现任何高级任务的关键环节——带来了重大挑战。本研究提出了一种新颖的多会话框架，该框架基于地图定位构建，有别于当前常见的贪婪式全SLAM会话运行及结果地图间对应关系查找的做法。我们的方法引入了拓扑感知、不确定性决策机制，通过分析位姿图结构来检测低连通性区域，并选择性触发建图与闭环检测模块。生成的增量地图与位姿图可无缝整合至现有模型中，有效减少累积误差并提升全局一致性。我们在数据集重叠序列上验证了该方法，并在真实矿井类环境中证明了其有效性。
...
- **🔒 开源**: 未提及
- **中文摘要**: 刚性夹爪与柔性夹爪的抓取合成方法之间存在表征差异。Anygrasp[1]及众多其他抓取合成方法均针对刚性平行夹爪设计，将其直接应用于柔性夹爪时往往无法捕捉其特有的顺应行为，导致模型训练数据需求量大且精度不足。为弥合这一差距，本文提出一种创新框架，将刚性夹爪模型的抓取姿态映射至柔性鳍射线夹爪。我们采用条件流匹配这一生成模型来学习这种复杂变换。研究方法包含构建数据采集流程以生成配对的刚性-柔性抓取姿态，通过U-Net自编码器从深度图像中提取物体几何特征作为CFM模型的条件输入，使其能够学习从初始Anygrasp姿态到稳定鳍射线夹爪姿态的连续映射。在七自由度机器人上的实验验证表明：使用柔性夹爪执行时，CFM生成的抓取姿态在已知物体和未知物体上的整体成功率（分别为34%和46%）显著优于基线刚性姿态（分别为6%和25%）。该模型在柱状物体（已知/未知物体成功率50%/100%）和球状物体（已知/未知物体成功率25%/31%）上表现尤为突出，并能有效泛化至未见物体。本研究证明CFM是一种数据高效且效果显著的抓取策略迁移方法，为其他软体机器人系统提供了可扩展的技术路径。
...
- **🔒 开源**: 未提及
- **中文摘要**: 相控阵蓝牙系统已成为在GNSS受限场景（如仓库物流、无人机着陆和自主对接）中实现辅助惯性导航的低成本替代方案。基于商用现成组件构建导航系统可降低相控阵无线电导航系统的应用门槛，但代价是测量噪声显著增加且有效作用距离相对较短。本文通过多旋翼无人机飞行实验数据，对比了基于因子图优化估计器的鲁棒估计策略。我们评估了在GNSS信号丢失情况下，借助蓝牙角度测量以及距离或气压测量辅助时的系统性能表现。
