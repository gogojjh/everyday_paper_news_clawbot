...
- **🔒 开源**: 未提及
- **中文摘要**: 对软体机器人等高维系统的控制，需要既能精确捕捉复杂动态特性又保持计算可行性的模型。本研究提出一种框架，将基于图神经网络（GNN）的动态模型与结构利用模型预测控制相结合，实现对高维系统的实时控制。通过将系统表示为具有局部交互作用的图结构，GNN保持了稀疏性，而定制化的凝聚算法则从控制问题中消除状态变量，确保计算效率。该凝聚算法的复杂度随系统节点数量线性增长，并利用图形处理器（GPU）并行化实现实时性能。所提方法在仿真和物理软体机器人躯干实验中均得到验证。结果表明，该方法可扩展至1000个节点系统在100赫兹闭环频率下运行，并在硬件上实现亚厘米精度的实时参考轨迹跟踪，性能较基线方法提升63.6%。最后，我们展示了该方法实现有效全身避障的能力。
...
### 4. Flickering Multi-Armed Bandits
...
### 6. MASPO: Unifying Gradient Utilization, Probability Mass, and Signal Reliability for Robust and Sample-Efficient LLM Reasoning
...
- **🔒 开源**: 未提及
- **中文摘要**: 道路上突然出现静态障碍物（即麋鹿测试）是自动驾驶避撞领域一个众所周知的紧急场景。模型预测控制（MPC）长期以来一直被用于先进自动驾驶车辆的规划与控制。然而，由于在此类危险场景中MPC进行规避动作的计算需求较高，麋鹿测试等紧急情况下的实时自动避撞实施问题仍未得到解决。本文通过实验实施MPC，在突然出现意外静态障碍物后进行运动规划，为实时避撞提供了新的见解。鉴于当前最先进的非线性MPC在实时提供可接受解决方案方面能力有限，我们提出了一种类似人类的前馈规划器，当MPC优化问题因初始猜测质量差而不可行或无法找到合适解决方案时提供辅助。我们引入最大转向操作概念来设计前馈规划器，模拟人类在检测到道路上静态障碍物后的反应。使用FPEV2-Kanon电动汽车在不同速度和紧急程度下进行了实际实验。此外，通过与最先进的MPC运动规划器进行比较，我们验证了所提出规划策略的有效性。

---

### 8. Distributed Virtual Model Control for Scalable Human-Robot Collaboration in Shared Workspace
...
- **🔒 开源**: 未提及
- **中文摘要**: 相控阵蓝牙系统已成为在仓库物流、无人机着陆和自主对接等全球导航卫星系统受限场景下，实现辅助惯性导航的低成本替代方案。基于商用现成组件构建导航系统虽会带来显著增加的测量噪声和相对有限的可行距离，但能有效降低相控阵无线电导航系统的应用门槛。本文通过分析多旋翼无人机飞行实验数据，对比了基于因子图优化估计器的鲁棒估计策略。研究评估了在蓝牙角度测量以及距离或气压测量辅助下，系统在全球导航卫星信号丢失场景中的性能表现。

---

### 10. Multi-session Localization and Mapping Exploiting Topological Information
...
- **🔒 开源**: 未提及
- **中文摘要**: 在已访问环境中进行操作对自主系统而言正变得日益重要，其在自动驾驶、测绘以及仓储或家庭机器人等领域具有直接应用价值。这种对同一区域的重复观测给建图与定位——实现任何高级任务的关键环节——带来了重大挑战。不同于当前普遍采用的贪婪式全SLAM会话及地图对应搜索策略，本研究提出了一种基于地图定位的新型多会话框架。该框架引入拓扑感知的智能决策机制，通过分析位姿图结构检测低连通区域，并选择性触发建图与闭环检测模块。生成的增量地图与位姿图将无缝整合至现有模型中，有效降低累积误差并提升全局一致性。我们在数据集重叠序列上验证了该方法，并在真实矿山环境中展示了其实际效能。

---

### 11. From Labor to Collaboration: A Methodological Experiment Using AI Agents to Augment Research Perspectives in Taiwan's Humanities and Social Sciences
...
- **🔒 开源**: 未提及
- **中文摘要**: 生成式人工智能正在重塑知识工作，但现有研究主要集中在软件工程和自然科学领域，对人文学科与社会科学的方法论探索尚显不足。本研究定位为"方法论实验"，提出一种基于智能体协作的人文学科与社会科学研究流程（智能体工作流）。研究以Anthropic经济指数中台湾地区Claude.ai使用数据（2025年11月，N=7,729组对话）为实证载体，验证该方法的可行性。

本研究在两个层面展开：主要层面是方法论框架的设计与验证——构建以任务模块化、人机分工、可验证性三原则为基础的七阶段模块化工作流，每个阶段明确人类研究者（研究判断与伦理决策）与智能体（信息检索与文本生成）的角色分工；次要层面是对台湾AEI数据的实证分析——作为工作流应用于二手数据研究的操作示范，展示流程执行过程与成果质量（参见附录A）。

本研究的贡献在于：为人文学科与社会科学研究者提出可复现的智能体协作框架，并通过操作过程的反思性记录，识别出人机协作的三种操作模式——直接执行、迭代优化、人类主导。这种分类揭示了人类在研究问题构建、理论阐释、情境化推理和伦理反思等方面不可替代的判断力。研究同时承认了单平台数据、横截面设计及人工智能可靠性风险等局限性。
