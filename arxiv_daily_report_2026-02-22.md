...
- **🔒 开源**: 未提及
- **中文摘要**: 对软体机器人等高维系统的控制，需要既能精确捕捉复杂动力学特性，又能保持计算可行性的模型。本研究提出一种将基于图神经网络（GNN）的动力学模型与结构利用模型预测控制相结合的框架，实现对高维系统的实时控制。通过将系统表示为具有局部交互作用的图结构，GNN保持了模型的稀疏性；同时采用定制化的凝聚算法从控制问题中消除状态变量，确保计算效率。该凝聚算法的复杂度随系统节点数量呈线性增长，并利用图形处理器（GPU）并行化技术实现实时性能。所提方法在仿真和实体软体机器人躯干实验中均得到验证。结果表明：该方法可扩展至1000个节点的系统，在100赫兹闭环控制频率下运行，并在硬件上实现亚厘米精度的实时参考轨迹跟踪，性能较基线方法提升63.6%。最后，我们展示了该方法实现全身有效避障的能力。
...
- **🔒 开源**: 未提及
- **中文摘要**: 在已访问环境中进行操作对自主系统而言正变得日益重要，其在自动驾驶、测绘以及仓储或家庭机器人等领域具有直接应用价值。这种对同一区域的重复观测给建图与定位——实现任何高级任务的关键环节——带来了重大挑战。本研究提出了一种创新的多会话框架，该框架基于地图定位构建，有别于当前普遍采用的贪婪式全SLAM会话运行及结果地图间对应关系查找的做法。我们的方法引入了拓扑感知、不确定性决策机制，通过分析位姿图结构来检测低连通性区域，并选择性地触发建图与闭环检测模块。生成的增量地图与位姿图将无缝整合至现有模型中，有效降低累积误差并提升全局一致性。我们在数据集的重叠序列上验证了该方法，并在真实矿井类环境中证明了其有效性。
...
- **🔒 开源**: 未提及
- **中文摘要**: 刚性夹爪与柔性夹爪的抓取合成方法之间存在表征差异。Anygrasp[1]及众多其他抓取合成方法专为刚性平行夹爪设计，将其直接应用于柔性夹爪往往无法捕捉其独特的顺应行为，导致模型需要大量数据且精度不足。为弥合这一差距，本文提出一种创新框架，将刚性夹爪模型的抓取姿态映射至柔性鳍条夹爪。我们采用生成模型条件流匹配（CFM）来学习这种复杂变换。研究方法包含构建数据采集流程以生成配对的刚性-柔性抓取姿态，通过U-Net自编码器从深度图像中提取物体几何特征作为CFM模型的条件输入，使其能够学习从初始Anygrasp姿态到稳定鳍条夹爪姿态的连续映射。我们在七自由度机器人上验证了该方法，实验表明：当柔性夹爪执行抓取时，相较于基线刚性姿态（对新旧物体的成功率分别为6%和25%），CFM生成的姿态对新旧物体实现了更高的综合成功率（分别为34%和46%）。该模型展现出显著改进，尤其对圆柱体（新旧物体成功率分别为50%和100%）和球体（新旧物体成功率分别为25%和31%）效果明显，并能成功泛化至未见物体。本研究证明CFM是一种数据高效且有效的抓取策略迁移方法，为其他软体机器人系统提供了可扩展的技术路径。
...
- **🔒 开源**: 未提及
- **中文摘要**: 相控阵蓝牙系统已成为在仓库物流、无人机着陆和自主对接等GNSS受限场景下实现辅助惯性导航的低成本替代方案。基于商用现成组件构建导航系统可降低相控阵无线电导航系统的应用门槛，但代价是测量噪声显著增加且有效作用距离相对较短。本文通过分析多旋翼无人机飞行实验数据，比较了基于因子图优化估计器的鲁棒估计策略。我们评估了在GNSS信号丢失情况下，借助蓝牙角度测量以及距离或气压测量辅助时的系统性能表现。
...
- **🔓 开源**: CODE
- **中文摘要**: 视觉同步定位与建图（V-SLAM）是实现自主感知与导航的基础能力。然而，内窥镜场景因持续的软组织形变而违背了刚性假设，导致相机自身运动与内在形变之间存在强烈的耦合模糊性。尽管近期单目非刚性SLAM方法取得了显著进展，但它们往往缺乏有效的解耦机制，并依赖稀疏或低保真度的场景表示，从而导致跟踪漂移和重建质量受限。为突破这些局限，我们提出NRGS-SLAM——一种基于3D高斯泼溅的内窥镜单目非刚性SLAM系统。为解决耦合模糊性问题，我们引入了形变感知的3D高斯地图，通过为每个高斯基元添加可学习的形变概率进行增强，并采用贝叶斯自监督策略进行优化，无需依赖外部非刚性标注。基于此表示方法，我们设计了可变形跟踪模块，通过优先处理低形变区域实现鲁棒的由粗到精姿态估计，继而执行高效的逐帧形变更新。精心设计的可变形建图模块能逐步扩展并优化地图，在表征能力与计算效率间取得平衡。此外，系统采用统一的鲁棒几何损失函数，融合外部几何先验知识以缓解单目非刚性SLAM固有的不适定性。在多个公开内窥镜数据集上的大量实验表明，相较于现有先进方法，NRGS-SLAM能实现更精确的相机姿态估计（均方根误差降低达50%）和更高质量的光照真实重建。全面的消融实验进一步验证了关键设计选择的有效性。源代码将在论文录用后公开。
...
- **🔒 开源**: 未提及
- **中文摘要**: 生成式人工智能对我们关于知识及其生产的理解提出了前所未有的挑战。与以往技术变革中工程理解先于或伴随部署的情况不同，生成式AI的运作机制在认识论层面仍具模糊性。缺乏这种理解，就无法在原则性基础上将其负责任地整合到科学、教育及制度生活中。本文认为，缺失的阐释必须始于一个尚未获得充分哲学关注的范式突破。在图灵-香农-冯·诺依曼传统中，信息以编码二进制向量形式进入机器，语义始终外在于该过程。神经网络架构打破了这一范式：符号输入被即时投射到高维空间中，其坐标对应语义参数，从而将二进制代码转化为意义几何空间中的位置。正是这个空间构成了塑造生成性生产的能动认识条件。本文基于高维几何的四个结构特性——测度集中、近正交性、指数级方向容量及流形正则性——构建了"高维空间的索引性认识论"。借鉴皮尔斯符号学与帕珀特建构主义理论，本文将生成模型重新概念化为习得流形的导航者，并提出导航性知识作为知识生产的第三种模式，既区别于符号推理，也不同于统计重组。
...
- **🔒 开源**: 未提及
- **中文摘要**: 现有可验证奖励强化学习（RLVR）算法（如GRPO）依赖于僵化、统一且对称的置信域机制，这些机制与大型语言模型（LLMs）复杂的优化动态存在根本性错配。本文指出此类方法面临三大关键挑战：（1）硬截断的二元截断机制导致梯度利用效率低下；（2）忽略词元分布的均匀比率约束引发概率质量敏感度不足；（3）正负样本间信用分配模糊度差异导致信号可靠性不对称。为弥合这些差距，我们提出质量自适应软策略优化（MASPO）——一个统一框架，旨在协调这三个维度。MASPO集成了可微分软高斯门控以最大化梯度效用，采用质量自适应限幅器平衡概率谱上的探索，并通过非对称风险控制器使更新幅度与信号置信度对齐。大量实验评估表明，MASPO作为稳健的一体化RLVR解决方案，显著优于现有基线方法。代码已开源：https://anonymous.4open.science/r/ma1/README.md。
